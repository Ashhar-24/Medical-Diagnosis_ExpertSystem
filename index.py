...
class DiseaseDiagnosis:
    ...
    def add_new_symptom(
        self, rule_name, symptom): from flask import Flask, render_template, request, jsonify, redirect
...
def addNewDisease():
    try:
        ...
        with open(os.path.join(engine.dataPath, 'disease-precaution.csv'), "a") as f:
            f.write(
                f"{disease_name.replace('_', ' ')},{','.join(disease_precautions)}\n")
        ...
        with open(os.path.join(engine.dataPath, 'disease-symptoms.clp'), "a") as f:
            ...
            f.write(f"  =>\n")
            f.write(
                f"  (printout t \"{disease_name.replace('_', ' ')}\" crlf)\n")
            f.write(f")\n")
            ...
            for symptom in new_symptoms:
                f.write(
                    f"  (has_symptom {symptom.strip().replace(' ', '_').lower()})\n")
            # f.write(f")\n")
            f.write(f"  =>\n")
